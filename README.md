...
| `src/lib/utils/provider-ref.ts` (new), `StackModal.svelte`, `GitStackModal.svelte`, `StackEnvVarsPanel.svelte`, `StackEnvVarsEditor.svelte`, `secret-providers/[id]/probe/+server.ts`, `secretproviders/{shared,probe-cache}.ts` | Client-side ref handling was hardcoded to `op://`; `GitStackModal` had no provider probe at all. Generalised to a per-provider scheme map, **added a per-row provider-ref badge** in the env editor (green found / spinner checking / amber not-found), added the probe to `GitStackModal`, made the probe endpoint check bulk + inline refs independently, dropped already-visible keys from the "injected at last deploy" banner, and added optional `probeReferences` / `listBulkKeys` contract hooks so the editor probe is one names-only request instead of N value fetches per keystroke. Upstream bugs — worth a standalone PR. |
